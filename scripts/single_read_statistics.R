...
colnames(data)<-c("chromosome","start","end","name","meth_sequence","Prob","context")
...
{
  methylation_calls <- unlist(strsplit(df[[5]], "[^mu]"))
  methylation_calls <- methylation_calls[methylation_calls != ""]
  binary <- as.integer(methylation_calls == "m")
  num <- length(binary);
  mu <- mean(binary);
  var <- var(binary);
  CV <- sqrt(var)/mu;
